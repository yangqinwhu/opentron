...
#     global tips_2,src_plates_2,src_tubes_2,trash_2,dest_plate_2
#     global tips,src_plates,src_tubes,trash,dest_plate,multi_pipette
#
...
#         src_plates = [protocol.load_labware_from_definition(saliva_rack,slot) for slot in rack_slots]
#         src_tubes = src_plates[0].rows()[0]+src_plates[1].rows()[0]
...
#         src_plates_2 = [protocol.load_labware_from_definition(saliva_rack,slot) for slot in rack_slots]
#         src_tubes_2 = src_plates_2[0].rows()[0]+src_plates_2[1].rows()[0]
...
#         rack_slots = ["6"]
#         tips = [protocol.load_labware(tip_name, slot) for slot in tip_slots]
#         src_plates = [protocol.load_labware_from_definition(saliva_rack,slot,"saliva rack batch 1 _micronic_96_wellplate_1400ul") for slot in rack_slots]
#         src_tubes=[]
#         for i in range(0,len(rack_slots)):
#             src_tubes += src_plates[i].rows()[0]
#         trash = protocol.load_labware_from_definition(liquid_trash_rack,trash_slot)
...
#
#         tips_2 = [protocol.load_labware(tip_name, slot) for slot in tip_slots_2]
#         src_plates_2 = [protocol.load_labware_from_definition(saliva_rack,slot,"saliva rack batch 2 _micronic_96_wellplate_1400ul") for slot in rack_slots]
#         src_tubes_2=[]
#         for i in range(0,len(rack_slots)):
#             src_tubes_2 += src_plates_2[i].rows()[0]
#         trash_2 = protocol.load_labware_from_definition(liquid_trash_rack,trash_slot_2)
...
class robot:
    def __init__(self,simulate =True,**kwarg):
        self.status=run_log()
        self.robot_initialize(simulate =simulate)
        self.assign_deck(**kwarg)
        self.load_deck()

    def robot_initialize(self,simulate =False):
        """
        connect, reset and home robot. create protocol variables to use.
        initialize is already built into load_deck(deck_plan).
        don't need to specifically use initialize.
    ...
    def load_deck(self):
        ...
        self.multi_pipette=pipette_class(multi_pipette,self.status)

        if self.trash_slot!="None":
            liquid_trash_rack=json.loads(self.lw.amsliquidtrash)
            self.trash = self.protocol.load_labware_from_definition(liquid_trash_rack,trash_slot)
...
class pipette_class:
    ...
    def p_transfer(self,s,d, b = 0,samp_vol= 50,air_vol = 25,mix=0, buffer_vol = 0,returnTip = False,chgTip=True,get_time = 1,disp=2,asp_bottom=2,disp_bottom=3,blowout = True,tip_presses = 1,tip_press_increment=0.3,reverse_vol=0,reverse_pip=0):
        """ s: source well  d: destination well b: buffer well.
        dispense: how many times the same to be dispensed
        Transfer from source well: s to destination well"""
        #set up pipette parameter
