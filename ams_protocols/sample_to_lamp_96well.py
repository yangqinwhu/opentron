...
def run_batch(start_tip=1,start_tube=1,start_dest=1,batch=1,samples=8,sample_per_column=8,replicates=1,aspirate_rate=0,dispense_rate=0,repl_chg_tip=False,rp4=False,**kwarg):
    """
    ...
    if rp4:
        ...
        dest_tubes_2 = ct.dest_plate_2.rows()[0]
    else:
    ...
    if rp4:
        dst_2 = dest_tubes_2[(start_dest-1):(sample_c*replicates+start_dest-1)]
    # print (len(sts))
    # print (len(dts))
    if len(sts)>len(dts):
    ...
    for i, (s, d) in enumerate(zip(sts,dts)):
        ...
        if rp4:
            ct.p_transfer(p,s,dst_2[i],**kwarg)
